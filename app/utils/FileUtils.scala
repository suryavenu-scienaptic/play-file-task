...
object FileUtils {
  ...
  def writeToFile(filePath: String, data: String): Try[Unit] =
    Try {
  ...
  def readFromFile(filePath: String): Try[List[Int]] =
    Try {
      val file = new File(filePath)
      ...
      case e: NumberFormatException =>
        Failure(new NumberFormatException("Invalid file content: Non-integer value found"))
      case e: IOException => Failure(new IOException(s"I/O error: ${e.getMessage}"))
      case e              => Failure(new Exception(s"Failed to read from file: ${e.getMessage}"))
    }

  def clearFile(filePath: String): Try[Unit] =
    Try {
      val file = new File(filePath)
      if (!file.exists()) throw new FileNotFoundException("File not found")
