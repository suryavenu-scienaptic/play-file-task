...
class FileController @Inject()(cc: ControllerComponents, fileService: FileService, errorHandler: ErrorHandler)(
  ...
  @ApiImplicitParams(
    Array(
      new ApiImplicitParam(value = "File request payload",
                           required = true,
                           dataType = "models.FileRequest",
                           paramType = "body")
    )
  )
    ...
    request.body.validate[FileRequest] match {
      case JsSuccess(fileRequest, _) =>
        ValidationUtils.validateFileRequest(fileRequest) match {
          case Success(_) =>
            fileService
              .saveToFile(fileRequest.start, fileRequest.end)
              .map { _ =>
